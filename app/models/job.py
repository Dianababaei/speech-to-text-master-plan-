...
class Job(Base):
    ...
    # Unique job identifier (stored as string in database)
    job_id = Column(String(100), nullable=False, unique=True, comment="Unique job identifier")

    # Status tracking
    status = Column(String(50), nullable=False, comment="Job status: pending, processing, completed, failed")
    ...
    # Additional metadata (mapped to 'metadata' column in database)
